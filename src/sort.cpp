...
void swap(ProcessManager::Proc* a, ProcessManager::Proc* b) {
  ProcessManager::Proc* temp = a;
  a = b;
  b = temp;
}

enum class Command { pid, name, state, mem, cpu, err };
...
std::vector<ProcessManager::Proc> sorting::get_sorted_procs(
        ...
        case Command::cpu:
          if (is_asc && processes[j].cpu_percent > processes[j + 1].cpu_percent)
            std::swap(processes[j], processes[j + 1]);
                   ...
                   processes[j].cpu_percent < processes[j + 1].cpu_percent)
            std::swap(processes[j], processes[j + 1]);
          break;
      }
    }
